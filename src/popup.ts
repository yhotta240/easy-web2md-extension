...
class PopupManager {
  ...
  private loadInitialState(): void {
    chrome.storage.local.get(["settings", "isEnabled"], (data) => {
      ...
      this.showMessage(
        this.isEnabled
          ? `${this.manifestData.name}${getMessage("enabledMessage")}`
          : `${this.manifestData.name}${getMessage("disabledMessage")}`,
      );
    });
  }
  ...
  private addEventListeners(): void {
    ...
    this.filenameCheckbox.addEventListener("change", (e: Event) => {
      ...
      this.showMessage(
        `${getMessage("filenameChanged")}${this.settings.fileName}${this.settings.saveFilename ? getMessage("rememberSuffix") : ""}${getMessage("changed")}`,
      );
  ...
  private clickURL(link: HTMLElement | string | null): void {
    ...
    const url =
      link instanceof HTMLElement && link.hasAttribute("href")
